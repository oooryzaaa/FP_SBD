class WishlistManager:
    ...
    def add_to_wishlist(self, customer_id, product_id):
        ...
        if not isinstance(customer_id, int) or not isinstance(product_id, int):
            return {"status": "error", "message": "Invalid customer or product ID"}
        if product_id not in [item["product_id"] for item in self.wishlist_items.get(customer_id, [])]:
            if customer_id not in self.wishlist_items:
                self.wishlist_items[customer_id] = []
            self.wishlist_items[customer_id].append({"product_id": product_id})
    ...
    def remove_from_wishlist(self, customer_id, product_id):
        ...
        if customer_id in self.wishlist_items:
            self.wishlist_items[customer_id] = [item for item in self.wishlist_items[customer_id] if item["product_id"] != product_id]
